...
static void printContactRow(int index, Contact &c) {
...
static void printContactDetails(Contact &c) {
...
static void handleAdd(PhoneBook &pb) {
  ...
  if (!readNonEmptyLine("First name: ", value))
    return;
  c.setFirstName(value);
  if (!readNonEmptyLine("Last name: ", value))
    return;
  c.setLastName(value);
  if (!readNonEmptyLine("Nickname: ", value))
    return;
  c.setNickname(value);
  if (!readNonEmptyLine("Phone number: ", value))
    return;
  c.setPhoneNumber(value);
  if (!readNonEmptyLine("Darkest secret: ", value))
    return;
...
static void handleSearch(PhoneBook &pb) {
  if (pb.size() == 0) {
